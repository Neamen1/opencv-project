...
int main(int argc, char* argv[])
{
    ...
    bool stream =false;
    int count_video_saves = 0;      //only for stream record
    ...
    while (1) {
        ...
        if (frame.empty()) {      // If the frame is empty, break
            ...
            if ( !myROI.empty() ) {                  //if ROI selected, showing cropped frame(+keys)
                ...
                else if (c == 122 and stream) {        //z  //start recording stream video 
    ...
    if (rec_start_frame>=0 and rec_stop_frame>=0 and !stream) {           //if time range is set and not stream - record
        ...
        cin >> var;
        cap.set(CAP_PROP_POS_FRAMES, rec_start_frame);  //going to rec start pos
        ...
        if (var == 0) {
            ...
            string outFileName = make_folder(filepath) + "\\cv_out.avi";
            ...
            string outFileName = make_folder(filepath) + "\\cv_croppedOut.avi";
